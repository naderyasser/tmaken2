...
import { DAYS, dayTotalMinutes, fmtHoursMinutes, windowsForDay, type CalendarType, type DayWindow, type ShiftKind } from './types'

/**
 * Apex's read-only day table (§5.7 / G-table): 7 rows Saturday→Friday, one
 * column-pair (حضور green / إنصراف red) per وردية for «دوام عادي», or a
 * single «ساعات الدوام» column for «دوام مفتوح». The pencil per row opens
 * the day dialog (G10) — this component owns no edit state itself.
 */
export function DayTable({
  kind, calendarType, windows, openHours, onEditDay,
}: {
  kind: ShiftKind
  calendarType: CalendarType
  windows: DayWindow[]
  openHours: number | string | null
        ...
        <table className="apex-table w-full border-collapse">
          <thead>
            <tr className="bg-[var(--apex-thead)]">
              ...
              <th className={th}>ساعات الدوام</th>
          ...
          <tbody>
            {DAYS.map((d) => (
              <tr key={d.value}>
                ...
                <td className={td}>{openHours === null || openHours === undefined || openHours === '' ? '_' : openHours}</td>
                <td className={td}>
            ...
            return (
              <tr key={d.value}>
                ...
                {[0, 1, 2, 3].map((i) => {
                  ...
                  return (
                    <Fragment key={i}>
                      <td className={td}>{w?.start_in || '_'}</td>
                      <td className={td}>{w?.end_out || '_'}</td>
                    </Fragment>
                  )
                })}
                <td className={td}>{fmtHoursMinutes(totalMinutes)}</td>
                <td className={td}>{isWorkDay ? 'عمل' : 'عطلة'}</td>
                <td className={td}>
                  <button type="button" onClick={() => onEditDay(d.value)} aria-label={`تعديل ${d.label}`} className="text-[var(--apex-link)] hover:opacity-70">
                    <Pencil className="h-4 w-4 mx-auto" />
                  </button>
