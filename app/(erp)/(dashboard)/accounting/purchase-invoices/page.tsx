...
import { accountingApi, type PurchaseInvoice } from '@/lib/accounting-api'
...
import { AlertCircle, RefreshCw, FileText, Search, Download } from 'lucide-react'
...
const L = {
    en: {
        ...
        subtitle: 'All purchase invoices from suppliers posted to the general ledger.',
        ...
        invoice: 'Invoice', supplier: 'Supplier', date: 'Date',
        dueDate: 'Due Date', grandTotal: 'Grand Total', outstanding: 'Outstanding',
        statusCol: 'Status', noData: 'No purchase invoices found.',
    ...
    ar: {
        ...
        subtitle: 'جميع فواتير المشتريات من الموردين المسجلة في دفتر الأستاذ.',
        ...
        invoice: 'الفاتورة', supplier: 'المورد', date: 'التاريخ',
        dueDate: 'تاريخ الاستحقاق', grandTotal: 'الإجمالي', outstanding: 'المتبقي',
        statusCol: 'الحالة', noData: 'لا توجد فواتير مشتريات.',
...
function getStatusInfo(inv: PurchaseInvoice, t: typeof L['en']) {
...
export default function PurchaseInvoicesPage() {
    ...
    // Status + search moved SERVER-side so pagination pages over the real result set.
    const statusExtraFilters = (): FrappeFilter[] => {
        switch (statusFilter) {
            case 'paid': return [['Purchase Invoice', 'docstatus', '=', 1], ['Purchase Invoice', 'is_return', '=', 0], ['Purchase Invoice', 'outstanding_amount', '<=', 0]]
            case 'unpaid': return [['Purchase Invoice', 'docstatus', '=', 1], ['Purchase Invoice', 'is_return', '=', 0], ['Purchase Invoice', 'outstanding_amount', '>', 0]]
            case 'cancelled': return [['Purchase Invoice', 'docstatus', '=', 2]]
            case 'return': return [['Purchase Invoice', 'is_return', '=', 1]]
            default: return []
        }
    }
    const buildOpts = useCallback((): Parameters<typeof accountingApi.getPurchaseInvoices>[0] => {
        const opts: Parameters<typeof accountingApi.getPurchaseInvoices>[0] = {
            from_date: fromDate,
            to_date: toDate,
        ...
        if (search) {
            opts.or_filters = [
                ['Purchase Invoice', 'name', 'like', `%${search}%`],
                ['Purchase Invoice', 'supplier_name', 'like', `%${search}%`],
                ['Purchase Invoice', 'supplier', 'like', `%${search}%`],
            ]
        }
        return opts
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const [data, count] = await Promise.all([
                accountingApi.getPurchaseInvoices({ ...opts, limit_start: (page - 1) * PAGE_SIZE, limit_page_length: PAGE_SIZE }),
                accountingApi.countPurchaseInvoices(opts),
            ])
            setRows(data)
            setTotal(count)
        } catch {
    ...
    // CSV covers ALL matching invoices, not just the page.
    const exportCsv = async () => {
        const all = await fetchAllPages<PurchaseInvoice>((start, len) =>
            accountingApi.getPurchaseInvoices({ ...buildOpts(), limit_start: start, limit_page_length: len }))
        const headers = ['Invoice', 'Supplier', 'Date', 'Due Date', 'Grand Total', 'Outstanding', 'Status']
        const csvRows = all.map(inv => {
            const st = getStatusInfo(inv, L['en'])
            return [inv.name, inv.supplier_name ?? inv.supplier, inv.posting_date, inv.due_date ?? '', fmt(inv.grand_total), fmt(inv.outstanding_amount), st.label]
        })
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-gray-50">
                                {[t.invoice, t.supplier, t.date, t.dueDate, t.grandTotal, t.outstanding, t.statusCol].map(h => (
                            ...
                            {loading ? [...Array(8)].map((_, i) => (
                                ...
                                return (
                                    <tr key={inv.name} className="border-b border-gray-50 hover:bg-gray-50/70 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-gray-600">{inv.supplier_name ?? inv.supplier}</td>
