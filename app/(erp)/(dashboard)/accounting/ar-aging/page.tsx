...
import { AlertCircle, RefreshCw, Users, Download } from 'lucide-react'
...
const L = {
    en: {
        ...
        subtitle: 'Outstanding customer balances grouped by how long they\'ve been overdue.',
        company: 'Company', asOf: 'As of Date', basis: 'Aging Based On',
        dueDate: 'Due Date', postingDate: 'Posting Date',
        run: 'Run Report', refresh: 'Refresh',
        customer: 'Customer', voucher: 'Voucher', posting: 'Posting Date',
        due: 'Due Date', invoiced: 'Invoiced', paid: 'Paid',
        outstanding: 'Outstanding', current: 'Current (0-30)',
        r2: '31-60 Days', r3: '61-90 Days', r4: '90+ Days',
        total: 'Total', noData: 'No outstanding receivables found.',
        error: 'Failed to load report.', loading: 'Loading...',
        allCompanies: 'Select company',
    },
    ar: {
        title: 'تقرير عمر الذمم المدينة',
        subtitle: 'أرصدة العملاء المستحقة مقسّمة حسب مدة الاستحقاق.',
        company: 'الشركة', asOf: 'بتاريخ', basis: 'أساس التقادم',
        dueDate: 'تاريخ الاستحقاق', postingDate: 'تاريخ الترحيل',
        run: 'تشغيل التقرير', refresh: 'تحديث',
        customer: 'العميل', voucher: 'المستند', posting: 'تاريخ الترحيل',
        due: 'تاريخ الاستحقاق', invoiced: 'المفوتر', paid: 'المدفوع',
        outstanding: 'المستحق', current: 'حالي (0-30)',
        r2: '31-60 يوم', r3: '61-90 يوم', r4: '90+ يوم',
        total: 'الإجمالي', noData: 'لا توجد ذمم مستحقة.',
        error: 'فشل تحميل التقرير.', loading: 'جاري التحميل...',
        allCompanies: 'اختر شركة',
    },
} as const
...
export default function ARAgingPage() {
    ...
    const runReport = useCallback(async () => {
        ...
        try {
            const data = await accountingApi.getARAgingReport({ company, report_date: asOf, ageing_based_on: basis })
    ...
    const totals = rows.reduce((acc, r) => ({
        invoiced: acc.invoiced + (r.invoiced ?? 0),
        paid: acc.paid + (r.paid ?? 0),
        outstanding: acc.outstanding + (r.outstanding ?? 0),
        r1: acc.r1 + (r.range1 ?? 0),
        r2: acc.r2 + (r.range2 ?? 0),
        r3: acc.r3 + (r.range3 ?? 0),
        r4: acc.r4 + (r.range4 ?? 0),
    ...
    return (
        <div dir={isRTL ? 'rtl' : 'ltr'} className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-wrap gap-4 items-end">
                ...
                <Button onClick={runReport} disabled={!company || loading} className="bg-emerald-600 hover:bg-emerald-700">
                ...
                {rows.length > 0 && (
                    <Button variant="outline" size="sm" onClick={() => exportToCsv(
                        `ar-aging-${asOf}.csv`,
                        ['Customer', 'Voucher', 'Due Date', 'Invoiced', 'Paid', 'Outstanding', '0-30', '31-60', '61-90', '90+'],
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    {[t.customer, t.voucher, t.due, t.invoiced, t.paid, t.outstanding, t.current, t.r2, t.r3, t.r4].map(h => (
